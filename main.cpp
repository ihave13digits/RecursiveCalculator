...
template <typename T> void Print(T data, std::string end="\n", int padding=0)
{
    ...
    else
    {
        ...
        for (int i=0; i<padding; i++) { pad_string=pad_string+" "; }
        if (end=="\n") { std::cout<<data<<std::endl; } else  { std::cout<<data<<pad_string<<end; }
    }
}
...
public:
    ...
    std::vector<std::string> Error(std::string text) { return {"Parentheses Error: "+text}; }
    ...
    std::string SolveProblem(std::string op, std::string A, std::string B)
    {
        ...
        float a=std::stof(A), b=std::stof(B);
        float c=0.0;
        if      (op=="^") { c=pow(a,b);}
        else if (op=="*") { c=    a*b; }
        else if (op=="/") { c=    a/b; }
    ...
    {
        int L=0, R=0;
        for (int i=0; i<tokens.size(); i++) { if (tokens[i]=="(") { L++; } else if (tokens[i]==")") { R++; } }
        if (L!=R) { return Error("Mismatched Parentheses"); }
        int _count = 2;
        while(_count>0)
        {
    ...
    std::string Parse(std::string data)
    {
        ...
        while(tokens.size()>1)
        {
            if (show_work) { PrintTokens(tokens); }
